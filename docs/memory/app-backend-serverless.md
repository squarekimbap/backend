...
**Key gotchas:** build needs `JAVA_HOME=$(/usr/libexec/java_home -v 21)` (Homebrew Maven pulls JDK 25). Keep project on ASCII path. Gov-data API normalization (items.item array/object/empty, resultCode "0000", mapX/mapx, serviceKey encoding) and coords→areaCd geocoding must be done server-side. TMAP/Elevation stay internal. New running flow is candidates → user picks → authenticated route-options → user picks → summary. Legacy routes keeps its response contract but is also authenticated. Both generators share a route-entry 22s deadline (8s reserved for JWT/cold start), 6/min/user quota and 5m cache; DDB calls time out at 2s and cache TTL is checked on read. See [[user-prefers-free-learn-by-doing]].
