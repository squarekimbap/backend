...
- 한국관광공사 TourAPI 음식점(type 39)을 조회해 `restaurant`/`cafe`로 분류하고 최대 8곳을 반환한다. 요청 반경에서 8개가 안 나오면 2.5km, 5km까지 자동 확장하며 응답의 `nearbyRadiusM`은 실제 반환 목록을 만든 반경이다.
...
코스 상세 화면의 "뛰고 나서 들를 곳" 영역용. 코스 `polyline`의 마지막 좌표인 **실제 완주 지점**을 기준으로 최대 8곳을 반환한다. 요청 반경에서 부족하면 2.5km, 5km까지 자동 확장하고 `radiusM`에는 실제 반환 목록을 만든 반경을 담는다. 상세 본문과 **병렬로 호출**하면 된다(느려도 본문은 먼저 뜬다).
