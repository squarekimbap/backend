...
- **lv·mood·scene·tags 는 정해진 어휘만 쓴다**(`validate_courses.py` 가 검사). 앱이 이 값으로 목록 배지와 필터를 만들어서, 새 값을 하나 넣으면 그 코스 혼자 있는 필터 칸이 생긴다. 늘리려면 앱과 먼저 맞출 것. `difficulty` 도 `ascentPerKm` 에서 자동 판정(10↓ 하 / 25↓ 중 / 초과 상)해 손으로 적은 값과 어긋나면 배포가 막힌다.
