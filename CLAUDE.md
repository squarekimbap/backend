...
- `GET /v1/courses/{id}/gpx` → 앱이 XML을 직접 만들지 않고 공유할 Garmin Connect 코스용 GPX 1.1 Track. `polyline`은 `trk/trkseg/trkpt`, 좌표가 있는 `poi`는 표준 `wpt`로 내보낸다. Garmin Connect가 외부 waypoint를 보존하지 않을 수 있으므로 경유지명 표시는 보장하지 않는다.
  ⚠️ **`@Produces`에 `MediaType.WILDCARD`를 반드시 함께 둘 것** — 앱 HTTP 클라이언트가 `Accept: application/json`을 전역으로 붙여서, 이게 없으면 GPX 요청이 통째로 **406**이 됐다(2026-09-04 실제 장애). curl은 Accept를 안 보내 재현되지 않고, 기존 테스트도 리소스 메서드를 직접 불러 협상을 건너뛰어 못 잡았다 — 회귀 테스트는 `RestAssured.given().accept(...)`로 HTTP를 타야 한다.
