...
- 코스 카드 본문에는 코스 거리 대신 `GET /v1/courses`의 `waypoints`를 최대 3곳까지 표시한다. `km`는 필터와 상세 수치에만 사용한다
...
- 앱이 GPX XML을 만들지 않는다. `GET /v1/courses/{id}/gpx` 응답 파일을 그대로 공유한다
- 서버 GPX는 GPX 1.1 `trk/trkseg/trkpt`이며 지나는 곳 `wpt`도 포함한다. Garmin Connect는 외부 waypoint를 보존하지 않을 수 있어 가민 화면의 지점명 표시는 보장하지 않는다
...
- GPX metadata의 관광공사·TMAP 출처 표기는 서버가 넣는다
